...
os.environ.setdefault("OCR_SERVER_DATABASE_URL", "postgresql://test:test@localhost/test")
...
async def client(db_engine, mock_storage):
    ...
    - Celery: process_pdf_task.apply_async is a no-op MagicMock
    """
    ...
    # Override the PDFService dependency to use test DB + mock storage
    async def override_get_pdf_service():
        async with factory() as session:
            repo = PDFRepository(session)
            service = PDFService(repository=repo, storage=mock_storage)
            try:
                yield service
    ...
    app.dependency_overrides[deps.get_pdf_service] = override_get_pdf_service

    # Patch Celery task to prevent real task dispatch
    with patch("api.v1.endpoints.upload.process_pdf_task") as mock_task:
